...
const handleChange = (e) => {
        ...
        if (formattedVal !== '-' && !formattedVal.endsWith('.')) {
            const numericVal = Number(formattedVal);
            if (!isNaN(numericVal)) {
                const boundedVal = Math.min(Math.max(min, numericVal), max);
                
                if (boundedVal !== numericVal) {
                    formattedVal = String(boundedVal);
                }
            }
        }
